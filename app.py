...
def submit_form(form_type):
    ...
    return render_template('submit_form.html', form_type=form_type, faculty=faculty)@app.route('/submit_form/<form_type>', methods=['GET', 'POST'])
@login_required@app.route('/submit_form/<form_type>', methods=['GET', 'POST'])
@login_required
@role_required(['student'])
def submit_form(form_type):
    if request.method == 'POST':
...
def submit_form(form_type):
    if request.method == 'POST':
        ...
        # Handle faculty selection for OD and Gate forms
        if form_type in ['od', 'gate']:
            faculty_emails = request.form.getlist('faculty')
            if not faculty_emails:
                flash('Please select at least one faculty for approval', 'error')
        ...
        if form_type == 'od':
            ...
            # Automatically assign to coordinator
            coordinator_ref = db.collection('users')\
                .where('role', '==', 'faculty')\
                .where('department', '==', session['department'])\
                .where('coordinator_section', '==', section_str)\
